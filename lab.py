...
def run_seasonal_ARIMA_experiment(db_conn, pred_year_from:int, pred_year_to:int):
    for ar_lag in range(1, 15):
        for ma_lag in range(3):
            for diff in range(3):
                method = md.SeasonalARIMA(db_conn=db_conn, AR_lag=ar_lag, MA_lag=ma_lag, differencing_order=diff, seasonal_period=6)
    ...
    for ar_lag in range(1, 15):
        for ma_lag in range(3):
            for diff in range(3):
                method = md.SeasonalARIMA(db_conn=db_conn, AR_lag=ar_lag, MA_lag=ma_lag, differencing_order=diff, seasonal_period=24)
                ex = Experimenter(db_conn=db_conn, method=method)
                ex.run(pred_year_from=pred_year_from, pred_year_to=pred_year_to)

def run_trend_residual_ARIMA_experiment(db_conn, pred_year_from:int, pred_year_to:int):
    for t_ar_lag in range(1, 12):
        for t_ma_lag in range(3):
            for t_diff in range(2):
                for r_ar_lag in range(1, 12):
                    for r_ma_lag in range(3):
                        for r_diff in range(2):
                            method = md.TrendResidualARIMA(db_conn=db_conn, trend_AR_lag=t_ar_lag, trend_MA_lag=t_ma_lag, trend_differencing_order=t_diff, 
                                                                            resid_AR_lag=r_ar_lag, resid_MA_lag=r_ma_lag, resid_differencing_order=r_diff)
                            ex = Experimenter(db_conn=db_conn, method=method)
                            ex.run(pred_year_from=pred_year_from, pred_year_to=pred_year_to)
...
if __name__ == "__main__":
    ...
    # run_seasonal_ARIMA_experiment(db_conn=conn, pred_year_from=2016, pred_year_to=2019)
    ...
    method.performance_test(pred_from_date=datetime(2018, 1, 1), pred_to_date=datetime(2018, 12, 1), 
